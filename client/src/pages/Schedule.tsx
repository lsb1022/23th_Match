...
export default function Schedule() {
  ...
  const { data: weeklySchedule, isLoading } = trpc.schedule.getCurrentWeek.useQuery();
              ...
              <div className="bg-gradient-to-r from-cyan-500/10 to-primary/10 p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">이번 주 날짜와 실제 담당자를 확인하세요</p>
                  </div>
                </div>
              </div>
            </Card>
                ...
                <Card key={day.date} className="elegant-card">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex items-center gap-3">
                      ...
                      <div>
                        <div>{day.dayName}요일</div>
                        <div className="text-xs font-normal text-muted-foreground mt-0.5">{day.date}</div>
                      </div>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
